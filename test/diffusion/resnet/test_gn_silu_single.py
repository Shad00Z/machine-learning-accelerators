...
@pytest.mark.skipif(not _cutile_available(), reason="cuda.tile / CUDA not available")
def test_gn_silu_single_kernel(ref):
    """The single-pass cuTile kernel matches the ref on real activations -- the M2 gate."""
    ...
    out = launch_single_pass_kernel(ref["x"], ref["weight"], ref["bias"], ref["num_groups"], ref["eps"])
